#include <ArduinoWebsockets.h>
#include <WiFi.h>
#include <HttpClient.h>
#include <ESPmDNS.h>
#include <ESPAsyncWebServer.h>
#include <Servo.h>
#include <HTTPClient.h>
#include "esp_camera.h"
#include "soc/soc.h"      
#include "soc/rtc_cntl_reg.h"
#include <PID_v1.h>
#include <iostream>
#include <sstream>
#include <string.h>

bool is_bouncing = false;
bool drawing_circle = false;
bool drawing_rect = false;

int index_html_gz_len = 3863;
const uint8_t index_html_gz[] = {
  
};


const char* ssid = "Gokce";
const char* password = "Nurcan123";

/*const char* ssid = "ballbouncer-ap";
const char* password = "BBouncer";*/
 
#define PWDN_GPIO_NUM     32
#define RESET_GPIO_NUM    -1
#define XCLK_GPIO_NUM      0
#define SIOD_GPIO_NUM     26
#define SIOC_GPIO_NUM     27
#define Y9_GPIO_NUM       35
#define Y8_GPIO_NUM       34
#define Y7_GPIO_NUM       39
#define Y6_GPIO_NUM       36
#define Y5_GPIO_NUM       21
#define Y4_GPIO_NUM       19
#define Y3_GPIO_NUM       18
#define Y2_GPIO_NUM        5
#define VSYNC_GPIO_NUM    25
#define HREF_GPIO_NUM     23
#define PCLK_GPIO_NUM     22
 
camera_fb_t * fb = NULL;
Servo servo1;
Servo servo2;
Servo servo3;
Servo servo4;
int servo1pin = 2;
int servo2pin = 14;
int servo3pin = 15;
int servo4pin = 13;

int current_servo1_angle;
int current_servo2_angle;
int current_servo3_angle;
int current_servo4_angle;

// PID values
double setpoint_x, input_x, output_x; //for X
double setpoint_y, input_y, output_y; //for Y

//PID const
float kp_x = 0.3;                                                     
float ki_x = 0.03;                                                      
float kd_x = 0.13;

float kp_y = 0.3;                                                       
float ki_y = 0.08;                                                      
float kd_y = 0.13;

PID PID_x(&input_x, &output_x, &setpoint_x, kp_x, ki_x, kd_x, DIRECT);
PID PID_y(&input_y, &output_y, &setpoint_y, kp_y ,ki_y ,kd_y, DIRECT);


using namespace websockets;
WebsocketsServer WSserver;
AsyncWebServer webserver(80);
HTTPClient http;

void WifiEvent(WiFiEvent_t event){
  switch(event){
    case SYSTEM_EVENT_STA_START:
      WiFi.setHostname("CSE396Group1");
      break;
  }
}
void setup() {
  WRITE_PERI_REG(RTC_CNTL_BROWN_OUT_REG, 0); //disable brownout detector
  Serial.begin(115200);
  
  camera_config_t config;
  config.ledc_channel = LEDC_CHANNEL_0;
  config.ledc_timer = LEDC_TIMER_0;
  config.pin_d0 = Y2_GPIO_NUM;
  config.pin_d1 = Y3_GPIO_NUM;
  config.pin_d2 = Y4_GPIO_NUM;
  config.pin_d3 = Y5_GPIO_NUM;
  config.pin_d4 = Y6_GPIO_NUM;
  config.pin_d5 = Y7_GPIO_NUM;
  config.pin_d6 = Y8_GPIO_NUM;
  config.pin_d7 = Y9_GPIO_NUM;
  config.pin_xclk = XCLK_GPIO_NUM;
  config.pin_pclk = PCLK_GPIO_NUM;
  config.pin_vsync = VSYNC_GPIO_NUM;
  config.pin_href = HREF_GPIO_NUM;
  config.pin_sscb_sda = SIOD_GPIO_NUM;
  config.pin_sscb_scl = SIOC_GPIO_NUM;
  config.pin_pwdn = PWDN_GPIO_NUM;
  config.pin_reset = RESET_GPIO_NUM;
  config.xclk_freq_hz = 20000000;
  config.pixel_format = PIXFORMAT_JPEG;
  //init with high specs to pre-allocate larger buffers
  if (psramFound()) {
    config.frame_size = FRAMESIZE_UXGA;
    config.jpeg_quality = 10;
    config.fb_count = 2;
  } else {
    config.frame_size = FRAMESIZE_SVGA;
    config.jpeg_quality = 12;
    config.fb_count = 1;
  }
   
  // camera init
  esp_err_t err = esp_camera_init(&config);
  if (err != ESP_OK) {
    Serial.printf("Camera init failed with error 0x%x", err);
    return;
  }
 
  sensor_t * s = esp_camera_sensor_get();
  s->set_framesize(s, FRAMESIZE_QVGA);

  //PID Initializations
  setpoint_x = 213;
  setpoint_y = 183;

  PID_x.SetMode(AUTOMATIC);
  PID_x.SetOutputLimits(30, 70);
  PID_y.SetMode(AUTOMATIC);
  PID_y.SetOutputLimits(30, 70);

  PID_x.SetSampleTime(50); 
  PID_y.SetSampleTime(50);  

  //Wifi start
  WiFi.disconnect(true);
  WiFi.onEvent(WifiEvent);
  WiFi.mode(WIFI_STA);
  WiFi.begin(ssid, password);


  // Wait for connection
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    Serial.print(".");
  }
 
  Serial.println("");
  Serial.print("Connected to ");
  Serial.print("IP address: ");
  
  //Set MDNS
  if(!MDNS.begin("BouncerESP32")){
    Serial.print("Error on MDNS");
    return;
  }
  Serial.println(WiFi.localIP());
  Serial.println(WiFi.getHostname());

  //TODO: Delete before delivering
  webserver.on("/", HTTP_GET, [](AsyncWebServerRequest * request) {
    AsyncWebServerResponse *response = request->beginResponse_P(200, "text/html", index_html_gz, sizeof(index_html_gz));
    response->addHeader("Content-Encoding", "gzip");
    request->send(response);
  });

  webserver.on("/bounce", HTTP_GET, [](AsyncWebServerRequest * request) {
    is_bouncing = !is_bouncing;
    drawing_circle = false;
    drawing_rect = false;

    AsyncWebServerResponse *response = request->beginResponse_P(200, "text/plain", "OK");
    request->send(response);
  });

  webserver.on("/draw_circle", HTTP_GET, [](AsyncWebServerRequest * request) {
     is_bouncing = false;
     drawing_rect = false;
     drawing_circle = !drawing_circle;
     AsyncWebServerResponse *response = request->beginResponse_P(200, "text/plain", "OK");
     request->send(response);
  });

  webserver.on("/draw_rectangle", HTTP_GET, [](AsyncWebServerRequest * request) {
     is_bouncing = false;
     drawing_circle = false;
     drawing_rect = !drawing_rect;
     AsyncWebServerResponse *response = request->beginResponse_P(200, "text/plain", "OK");
     request->send(response);
  });

  //Servo control with PID endpoint
  webserver.on("/servo_control",HTTP_GET, [](AsyncWebServerRequest * request){
    input_x = request->arg("x").toInt();
    input_y = request->arg("y").toInt();
    Serial.print("x is:");
    Serial.println(input_x);
    Serial.print("y is:");
    Serial.println(input_y);
    PID_x.Compute();  //action control X compute
    PID_y.Compute(); //   action control  Y compute   

    int servoangle_x = abs(servo1.read() - output_x);
    int servoangle_y = abs(servo2.read() - output_y);

    Serial.print("PID Output for x is: ");
    Serial.println(output_x);
    Serial.print("PID Output for y is: ");
    Serial.println(output_y);
  });

  //Editing PID Control Parameter Endpoint
  webserver.on("/pid_control",HTTP_GET, [](AsyncWebServerRequest * request){
    int req_ki_x = request->arg("ki_x").toInt();
    int req_kp_x = request->arg("kp_x").toInt();
    int req_kd_x = request->arg("kd_x").toInt();
    int req_ki_y = request->arg("ki_y").toInt();
    int req_kp_y = request->arg("kp_y").toInt();
    int req_kd_y = request->arg("kd_y").toInt();

    Serial.print(req_ki_x);
    Serial.print(req_kd_y);


    PID_x.SetTunings(req_kp_x, req_ki_x, req_kd_x);
    PID_y.SetTunings(req_kp_y, req_ki_y, req_kd_y);
    
  });


  Serial.print("Servo angles are initing...\n");

  // Initializing the servo motors with 30 degrees.
  servo1.attach(servo1pin, 2, 0, 180);
  servo1.write(30);

  servo2.attach(servo2pin, 3, 0, 180);
  servo2.write(30);
  
  servo3.attach(servo3pin, 4, 0, 180);
  servo3.write(30);

  servo4.attach(servo4pin, 5, 0, 180);
  servo4.write(30); 

  current_servo1_angle = 30;
  current_servo2_angle = 30;
  current_servo3_angle = 30;
  current_servo4_angle = 30;
  
  //Start http server and websocket
  webserver.begin();
  WSserver.listen(82);
  input_x = 0;
  input_y = 0;
}

void bounce() {
  servo1.attach(servo1pin, 2, 0, 180);
  servo1.write(30);
  
  servo2.attach(servo2pin, 3, 0, 180);
  servo2.write(30);
  
  servo3.attach(servo3pin, 4, 0, 180);
  servo3.write(30);
  
  servo4.attach(servo4pin, 5, 0, 180);
  servo4.write(30); 
  
  current_servo1_angle = 30;
  current_servo2_angle = 30;
  current_servo3_angle = 30;
  current_servo4_angle = 30;
  
  delay(250);
  
  servo1.attach(servo1pin, 2, 0, 180);
  servo1.write(70);
  
  servo2.attach(servo2pin, 3, 0, 180);
  servo2.write(70);
  
  servo3.attach(servo3pin, 4, 0, 180);
  servo3.write(70);
  
  servo4.attach(servo4pin, 5, 0, 180);
  servo4.write(70); 
  
  current_servo1_angle = 70;
  current_servo2_angle = 70;
  current_servo3_angle = 70;
  current_servo4_angle = 70;
  
  delay(250);
}

void draw_circle() {
  servo1.attach(servo1pin, 2, 0, 180);
  servo1.write(40);

  servo3.attach(servo3pin, 4, 0, 180);
  servo3.write(50);

  delay(200);

  servo2.attach(servo2pin, 3, 0, 180);
  servo2.write(40);
  
  servo4.attach(servo4pin, 5, 0, 180);
  servo4.write(50); 

  delay(200);

  servo1.attach(servo1pin, 2, 0, 180);
  servo1.write(50);

  servo3.attach(servo3pin, 4, 0, 180);
  servo3.write(40);

  delay(200);

  servo2.attach(servo2pin, 3, 0, 180);
  servo2.write(60);
  
  servo4.attach(servo4pin, 5, 0, 180);
  servo4.write(40); 

  delay(200);
}

void draw_rect(){
  servo1.attach(servo1pin, 2, 0, 180);
  servo1.write(30);

  servo3.attach(servo3pin, 4, 0, 180);
  servo3.write(70);

  delay(350);

  servo2.attach(servo2pin, 3, 0, 180);
  servo2.write(30);
  
  servo4.attach(servo4pin, 5, 0, 180);
  servo4.write(70); 

  delay(350);

  servo1.attach(servo1pin, 2, 0, 180);
  servo1.write(70);

  servo3.attach(servo3pin, 4, 0, 180);
  servo3.write(30);

  delay(350);

  servo2.attach(servo2pin, 3, 0, 180);
  servo2.write(70);
  
  servo4.attach(servo4pin, 5, 0, 180);
  servo4.write(30); 

  delay(350);
}
/*void pid_control(){
  HTTPClient http;
   http.begin("http://DESKTOP-9DCHT18-Wireless:3000/ball");
  int httpResponseCode = http.GET();
   if (httpResponseCode>0) {
        String payload = http.getString();
        std::vector<std::string> tokens;
        std::istringstream f(std::string(payload.c_str()));
        std::string s;
        while (getline(f, s, ',')) {
            tokens.push_back(s);
        }

        input_x = atoi(tokens[0].c_str());
        input_y = atoi(tokens[1].c_str());
      
        Serial.print("x is:");
        Serial.println(input_x);
        Serial.print("y is:");
        Serial.println(input_y);
              
        PID_x.Compute();  //action control X compute
        PID_y.Compute(); //   action control  Y compute   

        Serial.print("Output X: ");
        Serial.println(output_x);
        
        Serial.print("Output Y: ");
        Serial.println(output_y);
        
        int servoangle_x = current_servo1_angle - output_x;
        int servoangle_y = current_servo2_angle - output_y;
    
        Serial.print("servo_angle_x: ");
        Serial.println(servoangle_x);
        Serial.print("servo_angle_y: ");
        Serial.println(servoangle_y);
     
        servo1.attach(servo1pin, 2, 0, 180);
      
        int servo1angle;
        if(servoangle_x < 0){
           servo1angle = current_servo1_angle - servoangle_x;
        }
        else{
          servo1angle = current_servo1_angle + servoangle_x;
        }
        if(servo1angle > 70){
          servo1angle = 70;
        }
        else if(servo1angle < 30){
          servo1angle = 30;
        }
        current_servo1_angle = servo1angle;
        servo1.write(servo1angle);
    
       servo2.attach(servo2pin, 3, 0, 180);
       int servo2angle;
        if(servoangle_y < 0){
           servo2angle = current_servo2_angle - servoangle_y;
        }
        else{
          servo2angle = current_servo2_angle + servoangle_y;
        }
        if(servo2angle > 70){
          servo2angle = 70;
        }
        else if(servo2angle < 30){
          servo2angle = 30;
        }
       current_servo1_angle = servo2angle;
       servo2.write(servo2angle);

        
       servo3.attach(servo3pin, 4, 0, 180);
       int servo3angle;
      if(servoangle_x < 0){
         servo3angle = current_servo3_angle + servoangle_x;
      }
      else{
        servo3angle = current_servo3_angle - servoangle_x;
      }
      if(servo3angle > 70){
        servo3angle = 70;
      }
      else if(servo3angle < 30){
        servo3angle = 30;
      }
      current_servo3_angle = servo3angle;

      servo3.write(servo3angle);
    
      servo4.attach(servo4pin, 5, 0, 180);
      int servo4angle;
      if(servoangle_y < 0){
         servo4angle = current_servo4_angle + servoangle_y;
      }
      else{
        servo4angle = current_servo4_angle - servoangle_y;
      }       
      if(servo4angle > 70){
        servo4angle = 70;
      }
      else if(servo4angle < 30){
        servo4angle = 30;
      }
      current_servo4_angle = servo4angle;

       servo4.write(servo4angle); 
      delay(30);
 
   }
    else {
      Serial.print("Error code: ");
      Serial.println(httpResponseCode);
    }
      // Free resources
      http.end();
      
   
}*/
void loop() {
  auto client = WSserver.accept();
  while (client.available()) {
    //Sending image
    client.poll();
    fb = esp_camera_fb_get();
    client.sendBinary((const char *)fb->buf, fb->len);
    esp_camera_fb_return(fb);
    fb = NULL;  
    //pid_control();

    if (is_bouncing) {
      bounce();
    }
    if(drawing_circle){
      draw_circle();
    }
    if(drawing_rect){
      draw_rect();
    }
  }
}
